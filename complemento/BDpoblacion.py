...
NOMBRES_DB_CSV = ["0.csv", "0.CSV"]

# Repositorio público donde están las bases por municipio/provincia.
# El script intenta primero FILE.parquet y luego FILE.geoparquet,
# porque el repositorio actualmente usa archivos .geoparquet.
GITHUB_PUNTOS_RAW = "https://raw.githubusercontent.com/umayakuy/QGISCenso2024/main/data/puntos"
EXTENSIONES_PARQUET = [".parquet", ".geoparquet"]

TITULO_VENTANA = "CENSO 2024 - Base de Datos Municipal"
TITULO_MODULO = "Base de Datos Municipal (poblaciones)"
...
DATOS_CENSALES = {
    "Características principales de la población": [
        ...
        ("Población por lugar de nacimiento", "A2"),
        ("Población según residencia habitual", "A3"),
        ("Población que acude por problemas de salud", "A4"),
        ("Población según registro al SUS o afiliación a seguros de salud", "A5"),
        ("Población mayor a 19 años según instrucción de educación alcanzado", "A6"),
    ],
...
def normalizar_codigo(valor):
    return texto_limpio(valor).upper()
...
def descargar_archivo_github(file_codigo, cod_alternativo=None):
    """
    Descarga desde GitHub el archivo FILE.parquet o FILE.geoparquet.

    Por seguridad operativa, primero intenta con FILE de 4 caracteres
    y, si no existe en el repositorio, intenta con COD municipal de 5 caracteres
    cuando se envía cod_alternativo.
    ...
    carpeta_cache = os.path.join(tempfile.gettempdir(), "GeoCenso2024","poblaciones")
    ...
    for codigo in codigos:
        for extension in EXTENSIONES_PARQUET:
            ...
            try:
                ...
                # Si no existe .parquet, prueba .geoparquet; si tampoco, prueba el código alternativo.
                if error.code == 404:
                    continue
...
def es_capa_puntos(capa):
    if not isinstance(capa, QgsVectorLayer):
        return False
    return QgsWkbTypes.geometryType(capa.wkbType()) == QgsWkbTypes.PointGeometry
...
class Censo2024Dialog(QDialog):
    ...
    def analizar_capa(self, capa):
        """
        Valida que la capa sea de puntos, tenga COD y tenga al menos un COD válido relacionado.
        No genera tabla ni panel de resultados.
        """
        if not es_capa_puntos(capa):
            return None
    ...
    def seleccionar_capa(self):
        ...
        if not capas_validas:
            QMessageBox.information(
                self,
                "CENSO 2024",
                "El proyecto actual no tiene ningún mapa de municipio cargado.\n\n"
                "Para continuar, primero debe descargar un mapa de población por municipio mediante el complemento de GeoCenso2024"
                )
            self.close()
            return
    ...
    def obtener_parametros_preparados(self, codigo_variable):
        ...
        FILE = primeros 4 caracteres del COD municipal. Ejemplo: U502
        COD  = primeros 5 caracteres del COD municipal. Ejemplo: U5023
    ...
    def crear_tabla_filtrada_para_join(self, capa_datos, cod_municipal, db1, db2, nombre_tabla):
        """
        Crea una tabla temporal sin geometría con:
        - columna 1 del Parquet/GeoParquet, renombrada a COD;
        - columnas DB1 a DB2;
        - solo filas cuyo código de la columna 1 corresponde al COD municipal de 5 caracteres.
        """
        ...
        cod_municipal = normalizar_codigo(cod_municipal)
        nuevos_features = []
        total_revisados = 0

        for feature in capa_datos.getFeatures():
            total_revisados += 1
            atributos = feature.attributes()
            ...
            cod_dato = normalizar_codigo(atributos[0])

            # Se filtra por los primeros 5 caracteres del COD.
            # Ejemplo: U5023001 pertenece al municipio U5023.
            if not cod_dato.startswith(cod_municipal):
                continue

            valores = []
            for posicion, indice_origen in enumerate(indices):
                valor = atributos[indice_origen] if indice_origen < len(atributos) else None
                if posicion == 0:
                    valor = normalizar_codigo(valor)
                valores.append(valor)

            nuevo = QgsFeature(tabla.fields())
            nuevo.setAttributes(valores)
            nuevos_features.append(nuevo)

        if not nuevos_features:
            raise Exception(
                "El archivo descargado no contiene registros para el municipio {}.\n\n"
                "Registros revisados: {}".format(cod_municipal, total_revisados)
            )

        proveedor.addFeatures(nuevos_features)
        tabla.updateExtents()
    ...
    def aplicar_columnas_a_capa(self, tabla_datos, codigo_variable):
        """
        Copia los datos de la tabla preparada directamente a la capa de puntos seleccionada.

        Procedimiento:
        - No agrega la tabla Parquet/GeoParquet al panel de capas.
        - Usa COD de la capa seleccionada y COD de la tabla preparada.
        - Crea campos nuevos en la capa seleccionada usando el nombre original de la variable, sin prefijo del grupo censal.
        - Rellena los campos solo en las entidades cuyo COD coincida.
        """
        ...
        # 1) Preparar campos origen y nombres destino.
        ...
        for indice_origen, campo_origen in enumerate(campos_tabla):
            ...
            # Nombre final esperado, sin prefijo del grupo censal y sin sufijo _2.
            nombre_destino = nombre_campo_base(campo_origen.name())

            # Si el campo ya existe, se asume que esta BD ya fue cargada y se detiene
            # antes de crear columnas duplicadas como Edad_Total_T_2.
        ...
        # 2) Construir diccionario COD -> valores desde la tabla preparada.
        datos_por_cod = {}

        for feature in tabla_datos.getFeatures():
            atributos = feature.attributes()
            if indice_cod_tabla >= len(atributos):
                continue

            cod = normalizar_codigo(atributos[indice_cod_tabla])
            if not cod:
                continue

            valores = []
            for item in campos_a_copiar:
                indice_origen = item["indice_origen"]
                valor = atributos[indice_origen] if indice_origen < len(atributos) else None
                valores.append(valor)
        ...
        # 3) Agregar campos y rellenar valores en la capa seleccionada.
        capa = self.capa_seleccionada
        estaba_en_edicion = capa.isEditable()

        if not estaba_en_edicion:
            if not capa.startEditing():
                raise Exception(
                    "No se pudo activar la edición de la capa seleccionada.\n\n"
        ...
        try:
            ...
            for feature in capa.getFeatures():
                ...
                for indice_destino, valor in zip(indices_destino, valores):
                    if capa.changeAttributeValue(feature.id(), indice_destino, valor):
                        celdas_actualizadas += 1

            if registros_coincidentes == 0:
                raise Exception(
                    "No hubo coincidencias COD-COD entre la capa seleccionada y la base descargada.\n\n"
            ...
            if not estaba_en_edicion:
                if not capa.commitChanges():
                    ...
                    raise Exception(
                        "No se pudieron guardar los cambios en la capa seleccionada.\n\n{}".format(
                            errores
                        )
                    )
            ...
            if not estaba_en_edicion and capa.isEditable():
                capa.rollBack()
            raise
